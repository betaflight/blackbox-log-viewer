...
export function FlightLogParser(logData) {
  ...
  const parseFirmwareRevision = (fieldValue) => {
    ...
    const fwMatches = /((?:Beta|Race|Clean|Base|Butter)flight)\s+(\d+)\.(\d+)(?:\.(\d+))?/i.exec(
      fieldValue,
    )
    ...
    if (fwMatches != null) {
      this.sysConfig.firmwareType =
        FIRMWARE_NAME_MAP[fwMatches[1].toLowerCase()] ?? FIRMWARE_TYPE_BETAFLIGHT
  ...
  const accelLimitHandler = (fn, fv) => {
    ...
    const isCfModern =
      ...
      semver.gte(this.sysConfig.firmwareVersion, '2.0.0')
    this.sysConfig[fn] =
      isBfModern || isCfModern ? Number.parseInt(fv, 10) / 1000 : Number.parseInt(fv, 10)
  }
  ...
  const parseHeaderLine = () => {
    ...
    switch (fieldName) {
      case 'I interval':
        ...
        if (this.sysConfig.frameIntervalI < 1) this.sysConfig.frameIntervalI = 1
        break
      case 'P interval':
      ...
      case 'Firmware revision':
        ...
        if (matches != null) {
          ...
          this.sysConfig.firmware = parseFloat(matches[2] + '.' + matches[3]).toFixed(1)
          this.sysConfig.firmwarePatch = matches[5] != null ? parseInt(matches[5]) : '0'
          this.sysConfig.firmwareVersion =
            this.sysConfig.firmware + '.' + this.sysConfig.firmwarePatch
        } else {
          /*
           * Try to detect INAV
           */
          ...
          if (matches != null) {
            ...
            this.sysConfig.firmwarePatch = matches[5] != null ? parseInt(matches[5]) : ''
      ...
      default:
        if ((matches = fieldName.match(/^Field (.) (.+)$/))) {
          ...
          if (this.sysConfig.unknownHeaders == null) this.sysConfig.unknownHeaders = []
          this.sysConfig.unknownHeaders.push({ name: fieldName, value: fieldValue }) // Save the unknown headers
        }
        break
    }
  }

...
export function FlightLog(logData) {
  ...
  this.getSmoothedChunksInTimeRange = function (startTime, endTime) {
    ...
    let leadingROChunks = 1,
      trailingROChunks = 1
    let startIndex =
      binarySearchOrPrevious(iframeDirectory.times, startTime - maxSmoothing) - leadingROChunks
    let endIndex =
      binarySearchOrNext(iframeDirectory.times, endTime + maxSmoothing) + trailingROChunks
        ...
        mainLoop: for (
          ...
          for (
            ...
            while (
              ...
              resultChunks[centerChunkIndex - leadingROChunks].frames[centerFrameIndex][
                fieldIndex
...
FlightLog.prototype.gyroRawToDegreesPerSecond = function (value) {
  // Rotorflight logs gyro fields already converted to degrees per second, so no scaling is needed
  // (gyro_scale in Rotorflight logs is only used for IMU attitude integration, not field values).
  ...
  if (firmwareGreaterOrEqual(sysConfig, '3.0.0', '2.0.0')) {
    ...
    const calculateSetpointRate = function (axis, rc) {
      ...
      if (sysConfig.rates[axis]) {
        const rcSuperfactor =
          1.0 / constrain(1.0 - rcCommandfAbs * (sysConfig.rates[axis] / 100.0), 0.01, 1.0)
    ...
    const calculateRate = function (value, axis) {
      ...
      if (isSuperExpoActive()) {
        ...
        rcFactor =
          1.0 /
          constrain(1.0 - rcFactor * (validate(sysConfig.rates[axis], 100) / 100.0), 0.01, 1.0)
    ...
    if (axis === AXIS.YAW /*YAW*/) {
      ...
      return (
        (superExpoFactor *
          (((axis === AXIS.ROLL ? sysConfig.rates[AXIS.ROLL] : sysConfig.rates[AXIS.PITCH]) + 27) *
            value)) >>
        6
      )
    }

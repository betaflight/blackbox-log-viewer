...
GraphSpectrumCalc.initialize = function (flightLog, sysConfig) {
  ...
  this._blackBoxRate = rate;
  this._BetaflightRate = configuredRate;
  this._actualeRate = actualRate;
...
GraphSpectrumCalc.setOutTime = function (time) {
  if (time - this._analyserTimeRange.in <= MAX_ANALYSER_LENGTH) {
    ...
    this._analyserTimeRange.out =
      this._analyserTimeRange.in + MAX_ANALYSER_LENGTH;
  }
...
GraphSpectrumCalc.dataLoadPSD = function (_analyserZoomY) {
  ...
  const overlapCount =
    pointsPerSegment === totalCount
      ? 0
      : Math.floor((pointsPerSegment * 3) / 4);
...
GraphSpectrumCalc._dataLoadFrequencyVsX = function (
  vsFieldNames,
  minValue = Infinity,
  maxValue = -Infinity,
) {
  ...
  const fftChunkLength = Math.round(
    (this._blackBoxRate * FREQ_VS_THR_CHUNK_TIME_MS) / 1000,
  );
  const fftChunkWindow = Math.round(
    fftChunkLength / FREQ_VS_THR_WINDOW_DIVISOR,
  );
  ...
  for (
    ...
    const samples = flightSamples.samples.slice(
      fftChunkIndex,
      fftChunkIndex + fftChunkLength,
    );
    ...
    for (const vsValueArray of flightSamples.vsValues) {
      ...
      let sumVsValues = 0;
      for (
        let indexVs = fftChunkIndex;
        indexVs < fftChunkIndex + fftChunkLength;
        indexVs++
      ) {
...
GraphSpectrumCalc._dataLoadPowerSpectralDensityVsX = function (
  ...
  for (
    ...
    const fftInput = flightSamples.samples.slice(
      fftChunkIndex,
      fftChunkIndex + fftChunkLength,
    );
...
GraphSpectrumCalc.dataLoadPidErrorVsSetpoint = function () {
  ...
  return {
    fieldIndex: this._dataBuffer.fieldIndex,
    fieldName: this._dataBuffer.fieldName,
    axisName: FlightLogFieldPresenter.fieldNameToFriendly(
      `axisError[${axisIndex}]`,
    ),
...
GraphSpectrumCalc._getFlightChunks = function () {
  ...
  // Limit size
  logEnd =
    logEnd - logStart <= MAX_ANALYSER_LENGTH
      ? logEnd
      : logStart + MAX_ANALYSER_LENGTH;
...
GraphSpectrumCalc._getFlightSamplesFreqVsX = function (
  ...
  for (
    let fftChunkIndex = 0;
    fftChunkIndex + fftChunkLength < samplesCount;
    fftChunkIndex += fftChunkWindow
  ) {
 ...
 * For short data length, set pointsPerSegment same samples.length to extend samples count for power at 2 value inside _fft_segmented
 */
GraphSpectrumCalc._fft_segmented = function (
  samples,
  pointsPerSegment,
  overlapCount,
) {
  ...
  for (
    let i = 0;
    i <= samplesCount - pointsPerSegment;
    i += pointsPerSegment - overlapCount
  ) {

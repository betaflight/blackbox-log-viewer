<template>
  <div id="analyser" class="analyser">
    ...
    <input v-show="isFullscreen && showPsdHeatmap" v-model.number="maxPSD" type="number" :min="minPSD + 5" max="100" step="5"
      class="analyser-psd-input text-xs" :style="psdInputStyle(30)" @dblclick.ctrl="resetMaxPSD" />
    <label v-show="isFullscreen && showPsdHeatmap" class="analyser-psd-label text-xs text-dimmed" :style="psdLabelStyle(30)">
      Max&nbsp;dBm
    </label>
    <input v-show="isFullscreen && showPsdHeatmap" v-model.number="minPSD" type="number" min="-100" :max="maxPSD - 5" step="5"
      class="analyser-psd-input text-xs" :style="psdInputStyle(55)" @dblclick.ctrl="resetMinPSD" />
    <label v-show="isFullscreen && showPsdHeatmap" class="analyser-psd-label text-xs text-dimmed" :style="psdLabelStyle(55)">
      Min&nbsp;dBm
    </label>
    <input v-show="isFullscreen && showPsdHeatmap" v-model.number="lowLevelPSD" type="number" :min="minPSD" :max="maxPSD" step="5"
      class="analyser-psd-input text-xs" :style="psdInputStyle(80)" @dblclick.ctrl="resetLowLevelPSD" />
    <label v-show="isFullscreen && showPsdHeatmap" class="analyser-psd-label text-xs text-dimmed" :style="psdLowLevelLabelStyle">
      Limit&nbsp;dBm
    </label>

    <!-- PSD curve inputs (fullscreen only, PSD curve type) -->
    <input v-show="isFullscreen && showPsdCurve" v-model.number="segmentLength" type="number" min="6" :max="graphStore.segmentLengthMax" step="1"
      class="analyser-psd-input text-xs" :style="segmentInputStyle" @dblclick.ctrl="resetSegmentLength" />
    <label v-show="isFullscreen && showPsdCurve" class="analyser-psd-label text-xs text-dimmed" :style="segmentLabelStyle">
...
const showOverdrawSelect = computed(
  () => spectrumTypeNum.value !== SPECTRUM_TYPE.PIDERROR_VS_SETPOINT,
);
...
function resetZoomX() { zoomX.value = 100; getAnalyser()?.resetZoomX(); }
function resetZoomY() { zoomY.value = 100; getAnalyser()?.resetZoomY(); }
function resetMinPSD() { minPSD.value = userSettings.psdHeatmapMin; getAnalyser()?.resetMinPSD(); }
function resetMaxPSD() { maxPSD.value = userSettings.psdHeatmapMax; getAnalyser()?.resetMaxPSD(); }
function resetLowLevelPSD() { lowLevelPSD.value = minPSD.value; getAnalyser()?.resetLowLevelPSD(minPSD.value); }
function resetSegmentLength() { segmentLength.value = 9; getAnalyser()?.resetSegmentLength(); }
